    ...
    export default {
        mounted() {
            ...
            .then(resp => {
                ...
                app.form.code = resp.data.code
                app.form.description = resp.data.description
                app.form.points = resp.data.points
                app.form.level = resp.data.level
            })
            .catch(errors => alert('Could not load course'))
        },
        data() {
            return {
                form: new Form('/courses/' + this.$route.params.id, 'put', true, {
                    title: '',
                    code: '',
                    description: '',
                    points: '',
        ...
        methods: {
            onSubmit() {
                ...
                this.form.submit()
                    .then(data => app.$router.push({name: 'courses'}))
